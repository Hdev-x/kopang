...
export function WebAddressBook() {
  ...
  const openAdd = () => {
    setEditingId(null);
    // 첫 배송지는 기본 배송지가 없으면 곤란하므로 기본값을 켜둔다.
  ...
  const searchAddress = () => {
    if (!window.daum || !window.daum.Postcode) {
      window.alert("주소 검색 서비스를 불러오는 중이에요. 잠시 후 다시 시도해 주세요.");
      return;
    }
    new window.daum.Postcode({
      oncomplete: (data: { address: string; zonecode: string }) => setForm((current) => ({ ...current, zipcode: data.zonecode, address: data.address })),
    }).open();
  };
